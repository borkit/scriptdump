            ...
            if vpc['IsDefault'] == False:
                ...
                if vpc.get('Tags'):
                    for tag in vpc['Tags']:
                        if tag['Key'] == "Name":
                            vpc_name = tag['Value']
                print(Fore.GREEN + Style.BRIGHT + vpc_id + ' | ' + vpc_name + ' | ' + vpc['CidrBlock'] + ' | ' + count(instances, vpc_id)  + ' ec2 instances ' + Style.RESET_ALL)


#
#                 + count(dbs, vpc_id) + ' rds instances, ' + count(caches, vpc_id) + ' elastica\
